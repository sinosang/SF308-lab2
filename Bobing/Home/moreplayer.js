...
import yuebing from "../../assets/img/yuebing.jpg";
import xianyu from "../../assets/img/xianyu.jpg";
export default {
  setup() {
    ...
    if(changeplayer.value==0){
      touziresult.push(touzitemp_award.value);
    }
    if(changeplayer.value==1){
      touziresult2.push(touzitemp_award.value);
    }
      // touziresult.push(touzitemp_award.value);
   ...
   let changeplayer=ref(0);
    function nextone(state) {
     if(state==0){
      changeplayer.value=1;
      playerNow.value="player2"
     }
     if(state==1){
      changeplayer.value=0;
      playerNow.value="player1"
     }
     console.log(changeplayer.value)
    }
    const allPlayer=ref([
      {name:"player1",pic:yuebing},
      {name:"player2",pic:xianyu},

    ])
   let playerNow=ref("playe1")
    
    return {
      changeplayer,playerNow,
      allPlayer,
      touziresult,touziresult2,
      touzitemp_img,
      touziImg,
      touzitemp_num,
      touzitemp_award,
